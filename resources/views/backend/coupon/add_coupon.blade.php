...
@section('main')
        ...
        <div class="container">
            ...
            <form action="{{ route('store.coupon')}}" method="post"   class="row g-3 justify-content-center">
                @csrf
                <div class="col-md-5" >
                    <input type="text"  class="form-control shadow"  name="coupon_name"  placeholder="coupon Name">
                    @error('coupon_name')
        <div class="text-danger mt-1">{{ $message }}</div>
        @enderror
                </div>
                <div class="col-md-5">
                    <input type="text"  class="form-control shadow"  name="coupon_discount"  placeholder="coupon discount">
                    @error('coupon_discount')
        <div class="text-danger mt-1">{{ $message }}</div>
        @enderror
                </div>
                <div class="col-md-5">
                    <input type="date"  class="form-control shadow"  placeholder="Pcoupon validity"
